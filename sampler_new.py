...
class LangevinSampler(nn.Module):
    ...
    def get_grad(self, input, model_s, model_t, sample_label):
        ...
        loss = self.beta * loss_kd #self.gamma * loss_cls + self.alpha * loss_div + self.beta * loss_kd

        print(loss)

        grad_diff = torch.autograd.grad(loss, input, allow_unused=True, retain_graph=True)[0]
    ...
    def step(self, input, s_model, t_model, A):

        for i in range(self.n_steps):
            tmp_A = A.clone().requires_grad_()
            tmp_input = torch.zeros_like(input).to(self.device)

            for j in range(tmp_A.shape[0]):
                tmp_input[j] = (tmp_A[j] * input).sum(dim=1).mean()
            ...
            grad = self.get_grad(input, s_model, t_model, tmp_A).clamp_(-0.1, 0.1)

            import math
            tmp_A = tmp_A + grad * self.step_size / 2 + math.sqrt(self.step_size) * torch.randn(tmp_A.shape).to(self.device).normal_(0, 0.01)

            tmp_A = torch.nn.functional.normalize(tmp_A)

        return tmp_A


def get_samples(t_model, s_model, train_loader, args, class_num=100, sample_num=10000,
    ...
    while cnt < sample_num:
        ...
        input, _, _ = next(iter(train_loader))
        input = input.to(device)

        A = torch.diag(torch.ones(input_size[0])).to(device)
        ...
        for _ in range(steps):
            A = sampler.step(input, s_model, t_model, A)
    ...
    sorted_pairs = sorted_pairs[:int(sample_num * threshold)]
